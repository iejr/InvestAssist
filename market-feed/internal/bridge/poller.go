...
// rateSource observes a stablecoin->USD spot rate. Satisfied by
// *provider.KrakenREST; an interface so the poller is testable without a live
// exchange.
...
// Compile-time check that the real source satisfies the seam.
var _ rateSource = (*provider.KrakenREST)(nil)
...
func (e Edge) label() string { return e.Base + "/" + e.Quote }

// Poller samples stablecoin->USD bridge rates on a slow timer and writes them to
// latest_prices (newest edge) and price_candles (daily close). It is a
// self-clocked layer, deliberately separate from the streaming bus/sampler
...
func (p *Poller) pollAll(ctx context.Context) {
	for _, e := range p.edges {
		if err := p.pollOne(ctx, e); err != nil {
			log.Printf("bridge: %s: %v", e.label(), err)
		}
	}
}
...
func (p *Poller) pollOne(ctx context.Context, e Edge) error {
	price, ts, err := p.src.Ticker(ctx, e.Base, e.Quote)
	if err != nil {
		return err
	}
	...
	log.Printf("bridge: %s = %.6f (%s)", e.label(), price, day.Format("2006-01-02"))
	return nil
}
